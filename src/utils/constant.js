export const buttonArray=["All","News","Live","Tarak Mehta Ka Ooltah CHashmah","Shinchan","Ram Mantra","Krishna","Ramayan","Akshay Saini","Babbar","Scaler"]

export const CommentsData=[
          {
                    name:"Deepak Verma",
                    commentsinfo:"Kya bat Kya bat...Shandar..",
                    replies:[
                              {
                                        name:"Rahuk Verma",
                                        commentsinfo:"Jyada Ho gaya Itni Bhi Shandar Nhi...",
                                        replies:[
                                                  {
                                                         name:"Deepak Verma",
                                                         commentsinfo:"Tujhe Kya pata.."
                                                         
                                                  }
                                        ]      
                              },
                              {
                                        name:"Vidhya Shekahr",
                                        commentsinfo:"Thik Thak hai..",
                                        replies:[
                                                  {
                                                         name:"Akshay Deol"    ,
                                                         commentsinfo:"Shi Bat TotallY Agree" 
                                                  }
                                        ]      
                              }
                    ]
          },
          {
                    name:"Dwarka Verma",
                    commentsinfo:"Chal Mushafir Chalte Hai..",
                    replies:[
                              {
                                        name:"Rahuk Verma",
                                        commentsinfo:"Nhi Jaana...",
                                        replies:[
                                                  {
                                                         name:"Deepak Verma",
                                                         commentsinfo:"Jana Padega.."
                                                         
                                                  }
                                        ]      
                              },
                              {
                                        name:"Vidhya Shekahr",
                                        commentsinfo:"Chalo Phir..",
                                        replies:[
                                                  {
                                                         name:"Akshay Deol"    ,
                                                         commentsinfo:"Jindagi Ek Safar" 
                                                  }
                                        ]      
                              }
                    ]
          },
          {
                    name:"Dina Nath Chauhan",
                    commentsinfo:"Rishte ME toh hum tumhare Baap Lagte hai..",
                    replies:[
                              {
                                        name:"Rahuk Verma",
                                        commentsinfo:"Mera Toh nhi hai...",
                                        replies:[
                                                  {
                                                         name:"Deepak Verma",
                                                         commentsinfo:"Tera he Hai.."
                                                         
                                                  },
                                                  {
                                                            name:"pinky Verma",
                                                            commentsinfo:"Nikl Jaa l****.."
                                                            
                                                     },
                                        ]      
                              },
                              {
                                        name:"Shekahr Suman",
                                        commentsinfo:"Me hur pari..",
                                        replies:[
                                                  {
                                                         name:"Akshay Deol"    ,
                                                         commentsinfo:"Tu Jane Jaha" 
                                                  },
                                                  {
                                                            name:"Rakhi Deol"    ,
                                                            commentsinfo:"Tu Sabse Hsi Tu Sabse Juda" 
                                                     },
                                        ]      
                              }
                    ]
          }

]